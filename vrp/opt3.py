...
def reverse_segment_if_3better(tour, i, j, k, t, points):
    ...
    A, B, C, D, E, F = points[tour[i-1].index], points[tour[i].index], points[tour[j-1].index], points[tour[j].index], points[tour[k-1].index], points[tour[k % len(tour)].index]
...
def three_opt(tour, points):
    ...
    while True:
        ...
        for (a, b, c) in all_3segments(len(tour)):
            delta += reverse_segment_if_3better(tour, a, b, c, t, points)
            t = 0.9*t
        if delta >= 0:
            break 
        k += 1
